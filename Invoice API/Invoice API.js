...
module.exports = async function(app, con, client) {
    ...
    app.post('/extensions/invoiceApi/create', async function (req, res) {
        ...
        await con.query(`SELECT * FROM users WHERE userid="${req.body.user}"`, async(err, row) => {
            ...
            await con.query(`INSERT INTO invoices (userId, items, status, due, createdAt, username) VALUES ("${req.body.user}", '${arrayThing}', "Due", "${createdAt}", "${createdAt}", "${req?.body?.username}")`, async (err, row) => {
...
/*
 ...
 | | | |_   _ _ __   ___ _ __ _____| || |_ / _ \ / _ \ / _ \/ |
 | |_| | | | | '_ \ / _ \ '__|_  /_  ..  _| | | | | | | | | | |
 |  _  | |_| | |_) |  __/ |   / /|_      _| |_| | |_| | |_| | |
 |_| |_|\__, | .__/ \___|_|  /___| |_||_|  \___/ \___/ \___/|_|
        |___/|_|                                               
*/