...
module.exports = async function(app, con, client, faxstore) {
    faxstore.on('createUserAccount', async function(user, service) {
        ...
        if(request.data.length >= config.bansBeforeBlock) {
            ...
            await con.query(`UPDATE users SET disabled=1, banned=1, staffnotes="[FirewallGG] User ${user.userId} is actively banned in: ${list.join(', ')}." WHERE userId=${user.userId}`, async function(err, row) {
                if(err) throw err;
            });
        };
    });
